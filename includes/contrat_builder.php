...
/**
 * Construit le tableau $defaults pour buildContratHtml() à partir d'un agent
 * et (éventuellement) d'un contrat de la table `contrats`. Utilisé par
 * contrat.php (édition) et download_dossier.php (export groupé) afin que
 * les deux PDF générés pour un même contrat soient toujours identiques.
 */
function buildContratDefaults(PDO $db, array $a, array $c, array $params, int $agentId, bool $autoDetectPlanning = true): array {
    $defaults = [
        'civilite'             => 'M.',
    ...
    if ($autoDetectPlanning && (!$defaults['date_debut'] || !$defaults['date_fin'])) {
